    ...
    <main className="relative min-h-screen bg-gradient-to-b from-gray-950 via-[#0a1628] to-gray-950 text-white overflow-hidden">
      ...
      <section className="relative pt-32 md:pt-40 pb-14 md:pb-20">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <nav aria-label="Breadcrumb" className="mb-8 text-xs sm:text-sm text-white/40">
          ...
          <p className="text-lg md:text-xl text-white/65 max-w-3xl leading-relaxed mb-10">
            {lead}
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <Link
              href="/features"
              className="inline-flex items-center justify-center px-8 py-4 text-white border border-white/20 hover:border-white/40 text-base font-bold rounded-full transition-colors whitespace-nowrap"
            >
        ...
        >
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {section.body?.map((p, pi) => (
              <p
                key={pi}
                className="text-base md:text-lg text-white/65 leading-relaxed max-w-3xl mb-5"
              >
                {p}
              </p>
            ))}
            {section.bullets && section.bullets.length > 0 && (
              <ul className="grid sm:grid-cols-2 gap-3 mt-6 max-w-3xl">
                {section.bullets.map((b, bi) => (
                  <li
                    key={bi}
                    className="flex items-start gap-3 rounded-2xl bg-white/[0.03] border border-white/10 p-4"
                  >
                    <span className="inline-flex shrink-0 mt-0.5 w-5 h-5 rounded-full bg-cyan-400/20 text-cyan-300 items-center justify-center text-xs font-black">
                      ✓
                    </span>
                    <span className="text-white/80 text-sm md:text-base leading-snug">{b}</span>
                  </li>
                ))}
              </ul>
            )}
            {section.subsections && section.subsections.length > 0 && (
              <div className="grid md:grid-cols-2 gap-5 mt-8">
                {section.subsections.map((s) => (
                  ...
                  >
                    <h3 className="text-lg md:text-xl font-bold mb-3 text-white">{s.heading}</h3>
                    <p className="text-white/65 text-sm md:text-base leading-relaxed">{s.body}</p>
                  </div>
                ))}
              </div>
            )}
      ...
      {related.length > 0 && (
        <section className="relative py-20 md:py-24 bg-white/[0.02]">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <p className="text-[10px] sm:text-xs font-black tracking-[0.3em] uppercase text-cyan-300 mb-4">
              Passende Themen
            </p>
          ...
          <div className="relative rounded-3xl bg-gradient-to-br from-[#13203a] via-[#0d1a2e] to-[#13203a] border border-white/10 p-8 md:p-12 overflow-hidden">
            ...
            <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-8">
              <div className="max-w-xl">
                ...
                <p className="text-white/60 text-base md:text-lg">{ctaSubtitle}</p>
              </div>
              <Link
                href="https://signup.taskeyapp.com"
                target="_blank"
